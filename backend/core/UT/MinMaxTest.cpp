...
//---------------------------------Test: GenerateCandMmTest---------------------------------
class GenerateCandMmTest : public ::testing::TestWithParam<TestMovies>
{
    void SetUp()
    {
...
TEST_P(GenerateCandMmTest, TestName)
{
    // The scenario:
    ...
    {
        vector<IBoard::PositionXY> cands = m_minMax->generateCand();
...
//---------------------------------Test: UpdateCandMmTest---------------------------------
class UpdateCandMmTest : public ::testing::TestWithParam<TestMovies2>
{
    void SetUp()
    {
        m_board     = make_unique<GomokuBoard>(k_BOARD_SIZE);
...
TEST_P(UpdateCandMmTest, TestName)
{
    // The scenario:
    ...
    m_minMax->setStates(*m_board, *m_trackerCpu, *m_trackerHuman);
    SetBoard(*m_minMax->m_boardCpy.get());//score shall use m_boardCpy not original m_board
    const vector<IBoard::PositionXY> initCands = m_minMax->generateCand();
    ...
    m_minMax->m_boardCpy->putMove(newMove, extraPlayerMove);
    m_minMax->m_cpuCpy->updateScore(newMove, false, ThreatFinder::ThreatLocation::k_DEFAULT_MULTIPLIER);
    m_minMax->m_humanCpy->updateScore(newMove, true, ThreatFinder::ThreatLocation::k_DEFAULT_MULTIPLIER);

    if(not params.m_expectedMove.empty())
    {
        const vector<IBoard::PositionXY> updatedCands = m_minMax->updateCand(initCands,newMove);
    ...
    SetBoard(*m_minMax->m_boardCpy.get());//at the end set score to use original m_board
}

//---------------------------------Test: FindBestMoveMmTest---------------------------------
class FindBestMoveMmTest : public ::testing::TestWithParam<TestMovies3>
{
    void SetUp()
    {
        m_board = make_unique<GomokuBoard>(k_BOARD_SIZE);
        ...
        m_minMax = make_unique<MinMax>(k_DEFAULT_DEPTH, "MinMax");
        m_minMax->setStates(*m_board,*m_trackerCpu,*m_trackerHuman);
        m_minMax->setInitialPlayer(IBoard::Player::PLAYER_A);
    }
...
TEST_P(FindBestMoveMmTest, TestName)
{
    ...
    if(not params.m_expectedMove.empty())
    {
        ...
        {
            m_minMax->setStates(*m_board, *m_trackerCpu, *m_trackerHuman);
            m_minMax->setDepth(depth);
            nBestMove.clearAll();

            IBoard::PositionXY best = m_minMax->findBestMove(nBestMove);
...
//---------------------------------Test: StalemateMmTest---------------------------------
class StalemateMmTest : public ::testing::TestWithParam<TestMovies3>
{
    void SetUp()
    {
        m_board = make_unique<GomokuBoard>(k_BOARD_SIZE);
...
TEST_P(StalemateMmTest, TestName)
{
    TestMovies3 params = GetParam();
