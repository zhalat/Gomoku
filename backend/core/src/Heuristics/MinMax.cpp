...
void MinMax::setStates(const IBoard& board, const ThreatTracker& cpu, const ThreatTracker& human)
{
...
IBoard::PositionXY MinMax::findBestMove(PriorityQueueScore& bestMove, const vector<IBoard::PositionXY>& initCandidates)
{
    ...
    ScoreForMove retVal = gameTreeBrowsing(initCandidatesCpy, bestMove, treeTracker, true);
...
vector<IBoard::PositionXY> MinMax::generateCand() const
{
...
vector<IBoard::PositionXY> MinMax::updateCand(const vector<IBoard::PositionXY>& candidates, IBoard::PositionXY position) const
{
    const uint32_t boardSize = m_boardCpy->getSize();
...
void MinMax::updateCand4ExtendGaps(std::vector<IBoard::PositionXY>& candidates) const
{
    const uint32_t boardSize = m_boardCpy->getSize();
    VectorUniqueType gate;
    for(const auto el : candidates)
...
void MinMax::updateCand4ThreeListGaps(std::vector<IBoard::PositionXY>& candidates) const
{
    const uint32_t boardSize = m_boardCpy->getSize();
    VectorUniqueType gate;
    for(const auto el : candidates)
...
ISearchTree::ScoreForMove MinMax::gameTreeBrowsing( vector<IBoard::PositionXY>& candidates,
        ...
        else
        {
            for (const auto el: candidates)
            {
                ...
                else
                {
                    ...
                    // Go one move deeper.
                    leafScore = gameTreeBrowsing(nextCandidates, bestMovies, treeTracker, false, deep + 1);
                }
        ...
        else
        {
            for(const auto el: candidates)
            {
                ...
                else
                {
                    ...
                    // Go one move deeper.
                    leafScore = gameTreeBrowsing(nextCandidates, bestMovies, treeTracker, true, deep + 1);
                }
                ...
                m_treeLogger.AddEntryToRecord(deep, logger, log );
                m_treeLogger.AddEntryToRecord(deep, logger, *m_boardCpy );
                m_treeLogger.AddEntryToRecord(deep, logger, *m_cpuCpy );
                m_treeLogger.AddEntryToRecord(deep, logger, *m_humanCpy );
#endif
