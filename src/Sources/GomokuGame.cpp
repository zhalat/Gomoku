                      ...
                      const bool isRandomize, const uint32_t maxTime, std::istream & inStream, std::ostream & outStream)
{
    ...
    assert((size >= GomokuBoard::MIN_GOMOKU_BOARD_SIZE) && (size <= GomokuBoard::MAX_GOMOKU_BOARD_SIZE));
    ...
    // It will set default settings for the algo. but you still have control over algo. settings (like set deep,
    // tactics). Just use SearchTreeAlgorithmIf's interface.
...
bool GomokuGame::WinnerCheck(const Board::Player player) const
{
    ...
    if((m_pBoardScoreHuman->GetPlayer() == player) &&
       (m_pBoardScoreHuman->GetNumberOfRecognizedThreat(ThreatFinder::THREAT_WINNER)))
    {
        retVal = true;
    }
    else if((m_pBoardScoreCpu->GetPlayer() == player) &&
            (m_pBoardScoreCpu->GetNumberOfRecognizedThreat(ThreatFinder::THREAT_WINNER)))
    {
        retVal = true;
    }
    else
                                   ...
                                   const uint32_t initDepht) const
{
    ...
    while(!humanCandidates.empty())
    {
        ...
        const bool isNewMove = (std::find(existingCandidates.begin(), existingCandidates.end(), scoreForMove.m_move) ==
                                                ...
                                                const uint32_t diversityPercent) const
{
    ...
    for(; i < bestMoveSize; ++i)
    {
        ...
        const bool isStop =
            ((topMove.m_score < scoreMin) || (topMove.m_move == ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD));
