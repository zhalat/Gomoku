                                                             ...
                                                             const uint32_t deep)
{
    ...
    if(deep == m_DeepSearch)  // We reach the leaf.
    {
        ...
        {
            bestMove.m_score =
                (float)(m_pBoardScoreCpuCopy->GetScore()) - static_cast<int>(m_pBoardScoreHumanCopy->GetScore());
        }
        else
        {
            bestMove.m_score =
                -static_cast<int>((float)(m_pBoardScoreHumanCopy->GetScore())) + m_pBoardScoreCpuCopy->GetScore();
        }
    ...
    for(VectorUniqueTypeIterator pIterator = rCandidates.GetIteratorBegin(); pIterator != rCandidates.GetIteratorEnd();)
    {
        ...
        const bool isGameOver = m_pBoardScoreCpuCopy->GetNumberOfRecognizedThreat(ThreatFinder::THREAT_WINNER) > 0 ||
                                m_pBoardScoreHumanCopy->GetNumberOfRecognizedThreat(ThreatFinder::THREAT_WINNER) > 0;
