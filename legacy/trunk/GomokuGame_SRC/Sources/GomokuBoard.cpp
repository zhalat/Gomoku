...
GomokuBoard::GomokuBoard( uint32_t size )
{
    ...
    if ( size > MAX_GOMOKU_BOARD_SIZE || size < 1 ) throw s_throw;
...
bool GomokuBoard::RemoveObserver( const Observer::ObserverIf& rObserver )
{
    ...
    for ( std::vector< const Observer::ObserverIf* >::iterator it = m_ObserverClient.begin(); it != m_ObserverClient.end() ; ++it )
    {
        if( &rObserver == *it )
        {
...
void GomokuBoard::Announce() const
{
    ...
    for ( std::vector< const Observer::ObserverIf* >::const_iterator cit = m_ObserverClient.begin(); cit != m_ObserverClient.end(); ++cit )
    {
    	(*cit)->Update();
    }
}
