...
void BoardScore::GetGapsNonUnique( std::vector<Board::PositionField>& rGaps, const ThreatFinder::KindOfThreats threatKind ) const
{
	const uint32_t threatIndex = static_cast<uint32_t>( threatKind );

    // Get iterator to threat list:
    std::list<ThreatFinder::ThreatLocation>::const_iterator citer = m_ThreatsOnBoard.m_RecognizedThreats[ threatIndex ].begin();
...
void BoardScore::GetGapsUnique( std::vector<Board::PositionField>& rGaps, const ThreatFinder::KindOfThreats threatKind ) const
{
	const uint32_t threatIndex = static_cast<uint32_t>( threatKind );
	...
	for( ; citer != m_ThreatsOnBoard.m_RecognizedThreats[ threatIndex ].end(); ++citer )
	{
        ...
        for( uint32_t i = 0; i < ThreatFinder::ThreatUpDetails::MAX_EMPTY_SPACES; ++i  )
        {
        	...
        	// Gap must be unique.
        	const bool isGapOK = ( pretendGapXY != ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD ) && ( !addedGaps.IsPresent( pretendGap ) );
...
void BoardScore::GetGapsDuplicated( std::vector<Board::PositionField>& rGaps, const ThreatFinder::KindOfThreats threatKind ) const
{
	...
	for( std::vector<Board::PositionField>::const_iterator citer = nonUniqueGaps.begin(); citer < nonUniqueGaps.end(); ++citer )
	{
...
void BoardScore::GetExGaps( std::vector<Board::PositionField>& exGaps, const ThreatFinder::KindOfThreats threatKind ) const
{
	const uint32_t threatIndex = static_cast<uint32_t>( threatKind );
							   	   	   	   ...
							   	   	   	   const ThreatFinder::KindOfThreats threatKindB ) const
{
	...
	else
	{
	    ...
	    std::list<ThreatFinder::ThreatLocation>::const_iterator citerB = m_ThreatsOnBoard.m_RecognizedThreats[ threatIndexB ].begin();
	    ...
	    std::list<ThreatFinder::ThreatLocation>::const_iterator citerA = m_ThreatsOnBoard.m_RecognizedThreats[ threatIndexA ].begin();
...
uint32_t BoardScore::RemoveThreats( const Board::PositionXY xy )
{
    ...
    for ( uint32_t i = 0; i < Score::MAX_KIND_OF_THREATS; ++i )
    {
        ...
        {
        	std::list<ThreatFinder::ThreatLocation>::const_iterator citer = m_ThreatsOnBoard.m_RecognizedThreats[ i ].begin();
            ...
            for( ; citer != m_ThreatsOnBoard.m_RecognizedThreats[ i ].end(); )
            {
				...
				if ( isPartOfThreat )
				{
					...
					{
						// Get info required for reupadate threat. Must be able to keep up to 3 parts like: xxoxx or (rare situation) x.x..x.
						Board::PositionXY threatElements[ MAX_THREAT_PARTS ] = { XY_OUT_OF_BOARD, XY_OUT_OF_BOARD, XY_OUT_OF_BOARD };
						...
						for( uint32_t i = 0; i < MAX_THREAT_PARTS; ++i )
						{
							...
							if( XY_OUT_OF_BOARD != xyThreatElement )
							{
								...
								const Spotter::RegionToInvestigate regionToInvestigate { xyThreatElement, currentThreat, currentTrend };
					...
					else // There's necessity to run 'Promotion' threat group.
					{
						...
						GetThreatElementPromotion( threatLocation, xyThreatElement, GetThreatElementPromotionIndicator[ index ] );
...
bool BoardScore::AddThreats( const ThreatFinder::ThreatLocation& rTreatLocation, const uint32_t kindOfThreat, const uint32_t multiplier )
{
    bool retVal = false;
...
/// Provide threat list.
const std::list<ThreatFinder::ThreatLocation>& BoardScore::GetThreatList( const ThreatFinder::KindOfThreats threatKind ) const
{
	const uint32_t index = static_cast<uint32_t>( threatKind );
...
bool BoardScore::IsPartOfThreat( const ThreatFinder::ThreatLocation& rThreatLocation, const Board::PositionXY xy ) const
{
    ...
    {
        if( (rThreatLocation.m_ThreatDetails.m_BeginningThreat.m_x == xy.m_x) && (xy.m_y >= rThreatLocation.m_ThreatDetails.m_BeginningThreat.m_y) && (xy.m_y <=rThreatLocation.m_ThreatDetails.m_EndThreat.m_y) )
        {
            retVal = true;
        }
    }
    ...
    {
        if( (rThreatLocation.m_ThreatDetails.m_BeginningThreat.m_y == xy.m_y) && (xy.m_x >= rThreatLocation.m_ThreatDetails.m_BeginningThreat.m_x) && (xy.m_x <=rThreatLocation.m_ThreatDetails.m_EndThreat.m_x) )
        {
            retVal = true;
        }
    }
    else if ( rThreatLocation.m_Trend == ThreatFinder::RISING )
    {
        ...
        const int32_t b = rThreatLocation.m_ThreatDetails.m_BeginningThreat.m_y + rThreatLocation.m_ThreatDetails.m_BeginningThreat.m_x;
        const bool isOnLine = ( ( -xy.m_x + b ) == xy.m_y );
        const bool isInTheMiddle = ( rThreatLocation.m_ThreatDetails.m_BeginningThreat <= xy ) && ( xy <= rThreatLocation.m_ThreatDetails.m_EndThreat );
...
void BoardScore::GetThreatElementDismissal( const ThreatFinder::ThreatLocation& rThreatLocation, const Board::Player player, Board::PositionXY threatElements[ MAX_THREAT_PARTS ] ) const
{
...
/// In provided threat, search for any, non empty move, belongs to 'player' move.
void BoardScore::GetThreatElementPromotion( const ThreatFinder::ThreatLocation& rThreatLocation, Board::PositionXY& threatElement, const bool isTheFist ) const
{
	bool isFound = false;
