...
// Constructor.
Spotter::Spotter( BoardScore& rBoardScore )
 : m_rBoardScore( rBoardScore )
 , m_RegionToInvestigate()
{
...
{
#warning "Extract from this method. method 'ExecuteRegions()' and make it private. Execute() will run that metod and additionaly searches TWO_LIST threats."
	...
	for( ; pIteratorIfToExecute->HasNext(); )
	{
		...
		// Run blood relation
		IteratorIf<ThreatFinder::KindOfThreats>* const pIteratorIfBloodRelation = pThreatChanging->m_Dependency.GetIterator();
		...
		for( ; pIteratorIfBloodRelation->HasNext(); )
		{
			...
			const bool isFound = Score::m_ThreatScore[ threatIndex ].m_pThreat->FindThreatPattern( regionToInvestigate.m_Xy, regionToInvestigate.m_Trend, m_rBoardScore.GetPlayer() );
	...
	// If it was my move, besides previous stuff, run all Threat2CaseX (see ThreatsBloodRelation::TWO_LIST) because the new move might form a new threat.
	// Here is a place where each a new threat is born.
	if( !isOpponentMove )
	{
		for( uint32_t i = 0; i < ThreatsBloodRelation::TWO_LIST_SIZE; ++i )
		{
			...
			// Check if previously on specified trend any threat was found. If so, do not run it here again on the same trend.
			...
			if( isHorizontalRun )
			{
				isFoundHorizontal = Score::m_ThreatScore[ threatIndex ].m_pThreat->FindThreatPattern( xy, ThreatFinder::HORIZONTAL, m_rBoardScore.GetPlayer() );
			...
			if( isVerticalRun )
			{
				isFoundVertical = Score::m_ThreatScore[ threatIndex ].m_pThreat->FindThreatPattern( xy, ThreatFinder::VERTICAL, m_rBoardScore.GetPlayer() );
			...
			if( isRisingRun )
			{
				isFoundRising = Score::m_ThreatScore[ threatIndex ].m_pThreat->FindThreatPattern( xy, ThreatFinder::RISING, m_rBoardScore.GetPlayer() );
			...
			if( isFallingRun )
			{
				isFoundFalling = Score::m_ThreatScore[ threatIndex ].m_pThreat->FindThreatPattern( xy, ThreatFinder::FALLING, m_rBoardScore.GetPlayer() );
