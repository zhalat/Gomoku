...
Threat2CaseC::Threat2CaseC()
 : ThreatFinder( PATTERN_LENGHT, m_PointsView, NUMELEM( m_PointsView ), ATOM_NUMBER_2C )
{
	// nothing to do.
}

///	Check pretendThreat if meets all requirement to real threat.
bool Threat2CaseC::CheckThreat( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
...
void Threat2CaseC::GetThreatUpDetails( const Board::PositionXY initialPosition, const Trend trend, ThreatUpDetails& rThreatUpDetails ) const
{
	...
	const bool isPretendExtGap1OK = ( pretendExtGap1 <= rThreatUpDetails.m_Gaps[ 2 ] ) && ( GetGomokuBoard().IsOnBoard( pretendExtGap1 ) );
