/////////////////////////////////////////////////////////////////////////////////////////
/// @file Threat4CaseB.cpp
///
/// Threat4CaseB class definition.
///
/// @par Full Description.
/// Threat4CaseB finds pawns' which constitute one case of four pattern.
///
...
// C++ PROJECT INCLUDES
#include "Threat4CaseB.hpp"       // Threat4CaseB definition.

// FORWARD REFERENCES
// <none>

/// Tells how many pawns/gaps/asterixes this thrat has.
const std::unordered_map< ThreatFinder::ThreatAtom, int, std::hash<int> > Threat4CaseB::ATOM_NUMBER_4B =
{
...
const uint32_t Threat4CaseB::m_PointsView[] = { 1, 2, 3, 4, 5 };

Threat4CaseB::Threat4CaseB()
 : ThreatFinder( PATTERN_LENGHT, m_PointsView, NUMELEM( m_PointsView ), ATOM_NUMBER_4B )
{
	// nothing to do.
}

///	Check pretendThreat if meets all requirement to real threat.
bool Threat4CaseB::CheckThreat( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
...
bool Threat4CaseB::CheckThreatNormal( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
	...
	//2. Check enemy pawns.
	const bool isEnemyPawnsMatch = ( ( pretendThreat & adversaryPawnsCntrMask ) & ( threatPattern & adversaryPawnsCntrMask ) );
...
bool Threat4CaseB::CheckThreatSymmetric( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
	// Equal the first pointOfview in m_ThreatPattern
	static const uint32_t pointOfViewOffset 	= 1;
	...
	//1. Check my pawns(number and location).
	const uint32_t myPawnsCntr = __builtin_popcount ( pretendThreat & myPawnsCntrMask );
	const bool isMySymmetricPawnsMatch = ( ( pretendThreat & adversaryPawnsCntrMask ) & ( symmetricThreatPattern & adversaryPawnsCntrMask ) );

	//2. Check enemy pawns.
	const bool isEnemySymmetricPawnsMatch = ( ( pretendThreat & adversaryPawnsCntrMask ) & ( symmetricThreatPattern & adversaryPawnsCntrMask ) );
	...
	//4. Check asterix.
	const bool isAsterixMatch = ( ( pretendThreat & asterixCntrMask ) & ( threatPattern & asterixCntrMask ) );
	const bool isAsterixSymmetricMatch = ( ( pretendThreat & asterixCntrMask ) & ( symmetricThreatPattern & asterixCntrMask ) );
...
void Threat4CaseB::GetThreatUpDetails( const Board::PositionXY initialPosition, const Trend trend, ThreatUpDetails& rThreatUpDetails ) const
{
