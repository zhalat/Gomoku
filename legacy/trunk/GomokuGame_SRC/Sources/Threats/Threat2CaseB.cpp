...
Threat2CaseB::Threat2CaseB()
 : ThreatFinder( PATTERN_LENGHT, m_PointsView, NUMELEM( m_PointsView ), ATOM_NUMBER_2B )
{
	// nothing to do.
}

///	Check pretendThreat if meets all requirement to real threat.
bool Threat2CaseB::CheckThreat( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
...
bool Threat2CaseB::CheckThreatNormal( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
...
bool Threat2CaseB::CheckThreatSymmetric( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
	// Equal the first pointOfview in m_ThreatPattern
	static const uint32_t pointOfViewOffset = 1;
...
void Threat2CaseB::GetThreatUpDetails( const Board::PositionXY initialPosition, const Trend trend, ThreatUpDetails& rThreatUpDetails ) const
{
	...
	if( m_ThreatDownDetails.m_FoundFlags.m_IsNormalFound )
	{
		...
		else if( isExtGapsPattern2OK )	extGapIndex = 2;
		else if( isExtGapsPattern3OK )	extGapIndex = 1;
		else 							extGapIndex = -1;
	}
	else if( m_ThreatDownDetails.m_FoundFlags.m_IsSymmetricFound )
	{
		...
		if( isExtGapsPattern1OK )  		extGapIndex = 1;
		else if( isExtGapsPattern2OK )	extGapIndex = 0;
		else if( isExtGapsPattern3OK )	extGapIndex = 1;
		else 							extGapIndex = -1;
	}
	else
