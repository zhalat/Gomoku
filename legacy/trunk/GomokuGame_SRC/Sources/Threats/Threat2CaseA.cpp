/////////////////////////////////////////////////////////////////////////////////////////
/// @file Threat2CaseA.cpp
///
/// Threat2CaseA class definition.
///
/// @par Full Description.
/// Threat2CaseA finds pawns' which constitute one case of four pattern.
///
...
/// - zhalat 15-Nov-2017 Performance improvement.
///						 Threat starts provide its details ThreatUpDetails.
/// - zhalat 27-Nov-2017 All threat pieces location avaliable via GetThreatUpDetails().
/// @endif
///
/// @ingroup.
///
...
// C++ PROJECT INCLUDES
#include "Threat2CaseA.hpp"       // Threat2CaseA definition.
...
const ThreatFinder::ThreatPattern Threat2CaseA::m_ThreatPatternX[] =
{
	// Finds:
	// .xx...*
	// .x.x..*
...
const ThreatFinder::ThreatPattern Threat2CaseA::m_ThreatPatternO[] =
{
	// Finds:
	// .oo...*
	// .o.o..*
...
const uint32_t Threat2CaseA::m_ThreatPatternElements = NUMELEM( m_ThreatPatternX );
...
Threat2CaseA::Threat2CaseA()
 : ThreatFinder( PATTERN_LENGHT, m_PointsView, NUMELEM( m_PointsView ), ATOM_NUMBER_2A )
{
	m_FoundFlags.ClearAll();
}

///	Check pretendThreat if meets all requirement to real threat.
bool Threat2CaseA::CheckThreat( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
...
bool Threat2CaseA::CheckThreatNormal( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
	...
	const bool isGapsCntrMatch = ( ( pretendThreat & emptyCntrMask ) & ( threatPattern & emptyCntrMask ) ) == ( threatPattern & emptyCntrMask );

	//4. Check asterix.
	const bool isAsterixMatch = ( ( pretendThreat & asterixCntrMask ) & ( threatPattern & asterixCntrMask ) ) == ( threatPattern & asterixCntrMask );

	// Summarize.
	const bool isAllMatch = ( myPawnsCntr == myExpectedPawnInRow ) &&
												  isEnemyCntrMatch &&
									  ( gapsCntr >= expectedGaps ) &&
									   			   isGapsCntrMatch &&
												    isAsterixMatch;
...
bool Threat2CaseA::CheckThreatSymmetric( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
	// Equal the first pointOfview in m_ThreatPattern
	static const uint32_t pointOfViewOffset = 1;
	...
	//2. Check enemy pawns.
	const uint32_t enemySymmetricPawnsCntr = __builtin_popcount ( pretendThreat & ( symmetricThreatPattern & adversaryPawnsCntrMask  ) );
	const bool isEnemySymmetricCntrMatch = ( enemySymmetricPawnsCntr == 0 );

	//3. Check empty space.
	const uint32_t gapsCntr = __builtin_popcount ( pretendThreat & emptyCntrMask );
	const bool isGapsCntrSymmetricMatch = ( ( pretendThreat & emptyCntrMask ) & ( symmetricThreatPattern & emptyCntrMask ) ) == ( symmetricThreatPattern & emptyCntrMask );

	//4. Check asterix.
	const bool isAsteriSymmetricxMatch = ( ( pretendThreat & asterixCntrMask ) & ( symmetricThreatPattern & asterixCntrMask ) ) == ( symmetricThreatPattern & asterixCntrMask );

	// Summarize.
	const bool isAllSymmetricMatch = ( myPawnsCntr == myExpectedPawnInRow ) &&
									              isEnemySymmetricCntrMatch &&
									           ( gapsCntr >= expectedGaps ) &&
									               isGapsCntrSymmetricMatch &&
									                isAsteriSymmetricxMatch;
...
void Threat2CaseA::GetThreatUpDetails( const Board::PositionXY initialPosition, const Trend trend, ThreatUpDetails& rThreatUpDetails ) const
{
	...
	if( m_ThreatDownDetails.m_FoundFlags.m_IsNormalFound )
	{
		...
		rThreatUpDetails.m_Asterixes[ 0 ] = GetGomokuBoard().IsOnBoard( initialPositionNormTmp ) ? initialPositionNormTmp : ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD;
		...
		GetPieces( gapsHexCodeNormTmp, initialPositionNorm, directionForward, &rThreatUpDetails.m_Gaps[ 0 ], ThreatFinder::ThreatUpDetails::MAX_EMPTY_SPACES/2 );

		// c. Get extension gap. It's two movies from the second X. Must not be gather than Gaps[3].
		Board::PositionXY pretendExtGap = rThreatUpDetails.m_MyPawns[ 1 ];
		GetGomokuBoard().GoDirection( pretendExtGap, directionForward, 2 );
		const bool ispretendExtGapOK = ( pretendExtGap <= rThreatUpDetails.m_Gaps[ 3 ] ) && ( GetGomokuBoard().IsOnBoard( pretendExtGap ) );
		rThreatUpDetails.m_ExtGaps[ 0 ] = ( ispretendExtGapOK ) ? pretendExtGap : ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD;

		// d. Provide threat beginning & end. Threat's size must be greedy.
		rThreatUpDetails.m_BeginningThreat = rThreatUpDetails.m_Gaps[ 0 ];
		rThreatUpDetails.m_EndThreat = ( rThreatUpDetails.m_Asterixes[ 0 ] != ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD ) ? rThreatUpDetails.m_Asterixes[ 0 ] : rThreatUpDetails.m_Gaps[ 3 ];
	}

	if( m_ThreatDownDetails.m_FoundFlags.m_IsSymmetricFound )
	{
		// a. Get asterixes.
		rThreatUpDetails.m_Asterixes[ 1 ] = ( true == GetGomokuBoard().IsOnBoard( initialPositionNorm ) ) ? initialPositionNorm : ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD;

		// b. Get gaps. Keeps gaps in index 5-9.
		// 	  We don't want to count asterix as a gap. Remove that bit.
		uint8_t gapsHexCodeNormTmp = gapsHexCodeNorm;
		CLR_BIT( gapsHexCodeNormTmp, 0 );
		GetPieces( gapsHexCodeNormTmp, initialPositionNorm, directionForward, &rThreatUpDetails.m_Gaps[ ThreatFinder::ThreatUpDetails::MAX_EMPTY_SPACES/2 ], ThreatFinder::ThreatUpDetails::MAX_EMPTY_SPACES/2 );
		...
		const bool ispretendExtGapOK = ( pretendExtGap >= rThreatUpDetails.m_Gaps[ ThreatFinder::ThreatUpDetails::MAX_EMPTY_SPACES/2 ] ) && ( GetGomokuBoard().IsOnBoard( pretendExtGap ) );
		rThreatUpDetails.m_ExtGaps[ 1 ] = ispretendExtGapOK ? pretendExtGap : ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD;

		// d. Provide threat beginning & end. Threat's size must be greedy.
		const uint32_t lastGapSimmetricIndex = 3 + ThreatFinder::ThreatUpDetails::MAX_EMPTY_SPACES/2;
		rThreatUpDetails.m_BeginningThreat = ( rThreatUpDetails.m_Asterixes[ 1 ] != ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD ) ? rThreatUpDetails.m_Asterixes[ 1 ] : rThreatUpDetails.m_Gaps[ ThreatFinder::ThreatUpDetails::MAX_EMPTY_SPACES/2 ];
		rThreatUpDetails.m_EndThreat = ( ( rThreatUpDetails.m_Gaps[ lastGapSimmetricIndex ] > rThreatUpDetails.m_EndThreat ) ||
										 ( rThreatUpDetails.m_EndThreat == ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD ) ) ?  rThreatUpDetails.m_Gaps[ lastGapSimmetricIndex ] : rThreatUpDetails.m_EndThreat;
	}
