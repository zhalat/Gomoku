/////////////////////////////////////////////////////////////////////////////////////////
/// @file Threat3CaseB.cpp
///
/// Threat3CaseB class definition.
///
...
/// - zhalat 18-Feb-2017 Anomaly xxxx fix - enemy pawns taken into account.
...
// C++ PROJECT INCLUDES
#include "Threat3CaseB.hpp"       // Threat3CaseB definition.

// FORWARD REFERENCES
// <none>

/// Tells how many pawns/gaps/asterixes this thrat has.
const std::unordered_map< ThreatFinder::ThreatAtom, int, std::hash<int> > Threat3CaseB::ATOM_NUMBER_3B =
{
...
const uint32_t Threat3CaseB::m_PointsView[] = { 1, 2, 3, 4, 5 };

Threat3CaseB::Threat3CaseB()
 : ThreatFinder( PATTERN_LENGHT, m_PointsView, NUMELEM( m_PointsView ), ATOM_NUMBER_3B )
{
	// nothing to do.
}

///	Check pretendThreat if meets all requirement to real threat.
bool Threat3CaseB::CheckThreat( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
...
bool Threat3CaseB::CheckThreatNormal( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
	...
	const bool isMyPawnsMatch = ( ( pretendThreat & myPawnsCntrMask ) & ( threatPattern & myPawnsCntrMask ) ) == ( threatPattern & myPawnsCntrMask );

	//2. Check enemy pawns.
	const bool isEnemyPawnsMatch = ( ( pretendThreat & adversaryPawnsCntrMask ) & ( threatPattern & adversaryPawnsCntrMask ) ) == ( threatPattern & adversaryPawnsCntrMask );
...
bool Threat3CaseB::CheckThreatSymmetric( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
	// Equal the first pointOfview in m_ThreatPattern
	static const uint32_t pointOfViewOffset = 1;
	...
	//1. Check my pawns(number and location).
	const uint32_t myPawnsCntr = __builtin_popcount ( pretendThreat & myPawnsCntrMask );
	const bool isMySymmetricPawnsMatch = ( ( pretendThreat & myPawnsCntrMask ) & ( symmetricThreatPattern & myPawnsCntrMask ) ) == ( symmetricThreatPattern & myPawnsCntrMask );

	//2. Check enemy pawns.
	const bool isEnemySymmetricPawnsMatch = ( ( pretendThreat & adversaryPawnsCntrMask ) & ( symmetricThreatPattern & adversaryPawnsCntrMask ) ) == ( symmetricThreatPattern & adversaryPawnsCntrMask );

	//3. Check empty space.
	const uint32_t gapsCntrSymmetric = __builtin_popcount ( pretendThreat & emptyCntrMask & symmetricThreatPattern );

	//4. Check asterix.
	const bool isAsterixSymmetricMatch = ( ( pretendThreat & asterixCntrMask ) & ( symmetricThreatPattern & asterixCntrMask ) ) == ( symmetricThreatPattern & asterixCntrMask );

	// Summarize.
	const bool isAllSymmetricMatch = ( myPawnsCntr == myExpectedPawnInRow ) &&
									   isMySymmetricPawnsMatch &&
									   isEnemySymmetricPawnsMatch &&
									 ( gapsCntrSymmetric >= expectedGaps ) &&
									   isAsterixSymmetricMatch;
	return isAllSymmetricMatch;
}

/// Gets threat up details after threat has been found.
void Threat3CaseB::GetThreatUpDetails( const Board::PositionXY initialPosition, const Trend trend, ThreatUpDetails& rThreatUpDetails ) const
{
	...
	const uint8_t enemyPawnsHexCode = static_cast<uint8_t>( ( m_ThreatDownDetails.m_HexCode & adversaryPawnsCntrMask ) >> enemyPawnsShift );
	...
	const uint8_t enemyPawnsHexCodeNorm = StandarizePOV( enemyPawnsHexCode, m_ThreatDownDetails.m_PointOfView, PATTERN_LENGHT );
	...
	GetPieces( enemyPawnsHexCodeNorm, initialPositionNorm, directionForward, &rThreatUpDetails.m_EnemyPawns[ 0 ], ThreatFinder::ThreatUpDetails::MAX_ENEMY_PAWNS );
	...
	if( m_ThreatDownDetails.m_FoundFlags.m_IsNormalFound )
	{
		...
		if( isExtGapsPattern1OK )  		extGapIndex = 1;
		else 							extGapIndex = -1;
	}
	else if( m_ThreatDownDetails.m_FoundFlags.m_IsSymmetricFound )
	{
		...
		if( isExtGapsPattern1OK )  		extGapIndex = 0;
		else 							extGapIndex = -1;
	}
	else
	{
	...
	GetPieces( gapsHexCodeNormTmp, initialPositionNorm, directionForward, &rThreatUpDetails.m_Gaps[ 0 ], ThreatFinder::ThreatUpDetails::MAX_EMPTY_SPACES );

	//4. Provide extended gap.
	rThreatUpDetails.m_ExtGaps[ 0 ] = ( extGapIndex != -1 ) ? rThreatUpDetails.m_Gaps[ extGapIndex ] : ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD;
