...
ThreatWinner::ThreatWinner()
 : ThreatFinder( PATTERN_LENGHT, m_PointsView, NUMELEM( m_PointsView ), ATOM_NUMBER_WINNER )
{
	// nothing to do.
}

///	Check pretendThreat if meets all requirement to real threat.
bool ThreatWinner::CheckThreat( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
...
void ThreatWinner::GetThreatUpDetails( const Board::PositionXY initialPosition, const Trend trend, ThreatUpDetails& rThreatUpDetails ) const
{
