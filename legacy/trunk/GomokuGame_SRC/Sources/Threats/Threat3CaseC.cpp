/////////////////////////////////////////////////////////////////////////////////////////
/// @file Threat3CaseC.cpp
///
/// Threat3CaseC class definition.
///
/// @par Full Description.
/// Threat3CaseC finds pawns' which constitute one case of four pattern.
///
...
/// - zhalat 05-Dec-2016 Pattern recognizes additional case: x.x.x
/// - zhalat 12-Dec-2016 Pattern recognizes additional case: x..xx / xx..x
...
// C++ PROJECT INCLUDES
#include "Threat3CaseC.hpp"       // Threat3CaseC definition.

// FORWARD REFERENCES
// <none>

/// Tells how many pawns/gaps/asterixes this thrat has.
const std::unordered_map< ThreatFinder::ThreatAtom, int, std::hash<int> > Threat3CaseC::ATOM_NUMBER_3C =
{
...
const uint32_t Threat3CaseC::m_PointsView[] = { 1, 2, 3, 4, 5 };

Threat3CaseC::Threat3CaseC()
 : ThreatFinder( PATTERN_LENGHT, m_PointsView, NUMELEM( m_PointsView ), ATOM_NUMBER_3C )
{
	// nothing to do.
}

///	Check pretendThreat if meets all requirement to real threat.
bool Threat3CaseC::CheckThreat( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
	...
	// Summarize.
	const bool isAllMatch = isMyPawnsMatch &&
							isEnemyPawnsMatch &&
...
void Threat3CaseC::GetThreatUpDetails( const Board::PositionXY initialPosition, const Trend trend, ThreatUpDetails& rThreatUpDetails ) const
{
	...
	const bool isPretendExtGap1OK = ( pretendExtGap1 <= rThreatUpDetails.m_Gaps[ 1 ] ) && ( GetGomokuBoard().IsOnBoard( pretendExtGap1 ) );
	const bool isPretendExtGap2OK = ( pretendExtGap2 >= rThreatUpDetails.m_Gaps[ 0 ] ) && ( GetGomokuBoard().IsOnBoard( pretendExtGap2 ) );
