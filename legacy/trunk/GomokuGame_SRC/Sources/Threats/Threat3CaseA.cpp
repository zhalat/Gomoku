...
Threat3CaseA::Threat3CaseA()
 : ThreatFinder( PATTERN_LENGHT, m_PointsView, NUMELEM( m_PointsView ), ATOM_NUMBER_3A )
{
	m_FoundFlags.ClearAll();
}

///	Check pretendThreat if meets all requirement to real threat.
bool Threat3CaseA::CheckThreat( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
...
bool Threat3CaseA::CheckThreatNormal( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
...
bool Threat3CaseA::CheckThreatSymmetric( const uint32_t pretendThreat, const uint32_t pointOfView, const Board::Player playerPerspective ) const
{
	// Equal the first pointOfview in m_ThreatPattern
	static const uint32_t pointOfViewOffset = 1;
...
void Threat3CaseA::GetThreatUpDetails( const Board::PositionXY initialPosition, const Trend trend, ThreatUpDetails& rThreatUpDetails ) const
{
	...
	if( m_ThreatDownDetails.m_FoundFlags.m_IsNormalFound )
	{
		...
		const bool ispretendExtGapOK = ( pretendExtGap <= rThreatUpDetails.m_Gaps[ 2 ] ) && ( GetGomokuBoard().IsOnBoard( pretendExtGap ) );
		...
		rThreatUpDetails.m_EndThreat = ( rThreatUpDetails.m_Asterixes[ 0 ] != ThreatFinder::ThreatLocation::XY_OUT_OF_BOARD ) ? rThreatUpDetails.m_Asterixes[ 0 ] : rThreatUpDetails.m_Gaps[ lastGapIndex ];
	}
