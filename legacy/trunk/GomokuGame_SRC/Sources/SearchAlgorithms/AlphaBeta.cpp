...
static void nBestMoveLogger( const uint32_t deep,
						     SearchTreeAlgorithmIf::PriorityQueueScore& rPriorityQueueScore,
...
Board::PositionXY AlphaBeta::FindBestMove( PriorityQueueScore& rBestMove, const vector<Board::PositionXY>& rInitCandidates )
{
    using namespace std;
      	...
      	else
      	{
			...
			for ( VectorUniqueTypeIterator pIterator = rCandidates.GetIteratorBegin(); pIterator != rCandidates.GetIteratorEnd(); )
			{
				...
				else
				{
					...
					// Go one move deeper.
					leafScore = GameTreeBrowsing( nextCandidates, rBestMovesPriorityQueue, rTreeTracker, alpha, beta, false, deep + 1 );
				}
      	...
      	else
      	{
			...
			for ( VectorUniqueTypeIterator pIterator = rCandidates.GetIteratorBegin(); pIterator != rCandidates.GetIteratorEnd(); )
			{
				...
				else
				{
					...
					// Go one move deeper.
					leafScore = GameTreeBrowsing( nextCandidates, rBestMovesPriorityQueue, rTreeTracker, alpha, beta, true, deep + 1 );
				}
