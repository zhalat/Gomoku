...
static Board::PositionField FindCommonMove( const std::vector<Board::PositionField>& rGapsA, const std::vector<Board::PositionField>& rGapsB );

/// Get all common elements.
static std::vector<Board::PositionField> FindCommonMovements( const std::vector<Board::PositionField>& rGapsA, const std::vector<Board::PositionField>& rGapsB );
...
bool StateEvaluation::ImpliciteLooser( const bool isMaxPlayer, Board::PositionXY& rBuildUpMove, int& rStateScore ) const
{
	...
	else if( HeadShotRun( HEAD_SHOOT_DOUBLE_3A_ONE_STROKE, !isMaxPlayer, temp, true ) && // shall be also !isMy3A && ' the same as lizard'
	...
	else if( HeadShotRun( HEAD_SHOOT_DRAGON, !isMaxPlayer, temp, true ) )
	{
		...
		retVal = true;
			 ...
			 HeadShotRun( HEAD_SHOOT_LIZARD, !isMaxPlayer, temp, true ) )
	{
		rStateScore = adversarySign * ScoreEval( this, MILESTONE_SCORE );
		retVal = true;
	}
...
bool StateEvaluation::ExtendWinnerThreatMove( const bool isMaxPlayer, Board::PositionXY& rBuildUpMove, int& rStateScore ) const
{
	bool retVal = false;
	int mySign = 0;
	int adversarySign = 0;
...
bool StateEvaluation::ExtendWinnerActionMove( const bool isMaxPlayer, Board::PositionXY& rBuildUpMove, int& rStateScore ) const
{
	...
	else if( IsDouble3AOneStrokePossible( isMaxPlayer, rBuildUpMove ) &&
			 !IsHeadShot3APossible( !isMaxPlayer, buildUpTmp ) &&
			 !IsHeadShot3AAPossible( !isMaxPlayer, buildUpTmp ) &&
			 !IsHeadShot4BCPossible( !isMaxPlayer, buildUpTmp ) &&
...
bool StateEvaluation::HeadShotRun( const HeadShotAction headShotAction,
								   const bool isMaxPlayer,
								   Board::PositionXY& rShotMove,
...
bool StateEvaluation::IsHeadShot4BCPossible( const bool isMaxPlayer, Board::PositionXY& rBuildUpMove, const bool checkWinnerMoveCondition,
																									  ...
																									  const BoardScore* pHumanBoardScore ) const
{
	...
	const bool isAdversary4ABC  = ( pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_A ) > 0 ||
	...
	const bool isHeadShootPossibleStage1 = ( pMyBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_AA ) +
...
bool StateEvaluation::IsHeadShot3APossible( const bool isMaxPlayer, Board::PositionXY& rBuildUpMove, const bool checkWinnerMoveCondition,
																									 const BoardScore* pCpuBoardScore,
																									 const BoardScore* pHumanBoardScore ) const
{
	bool retVal = false;
	...
	const uint32_t my4BCNumberOfThreatsInit = pMyBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_AA ) +
											  pMyBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_B ) +
											  pMyBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_C );
	...
	if( isHeadShootPossibleStage1 )
	{
		...
		for( auto iter = commonMovements.begin(); iter != commonMovements.end(); ++iter )
		{
			...
			const uint32_t my4BCNumberOfThreatsStage1	= ( pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_AA ) +
			...
			const bool isHeadShootPossibleStage2 = ( my4BCNumberOfThreatsStage1 > my4BCNumberOfThreatsInit ) &&
			...
			if( isHeadShootPossibleStage2 )
			{
				if( isAdversary3A )
				{
					...
					if( commonMoveStage2 != POSITION_OUT_OF_BOARD )
					{
						...
						const uint32_t my4BCNumberOfThreatsStage2 = ( pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_AA ) +
																	  pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_B )  +
																	  pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_C ) );

						// Only adverary's 4A threat may shut down my attack. 4AA is not included.
						// My 3A threat must exist.
						const uint32_t adversary4ANumberOfThreatsStage2 = pAdversaryBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_A );

						const bool isMyAttackFail = ( ( ( my4BCNumberOfThreatsStage2 < my4BCNumberOfThreatsStage1 ) && ( adversary4ANumberOfThreatsStage2 > 0 ) ) ||
													  ( pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A ) ) == 0 );
			...
			if( retVal )
			{
				...
				const uint32_t my3ACNumberOfThreatsStage3 = pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A );
				const uint32_t my4BCNumberOfThreatsStage3 = ( pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_AA ) +
															  pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_B )  +
															  pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_C ) );
				...
				for( auto iter = commonMovements.begin(); iter != commonMovements.end(); ++iter )
				{
					...
					const uint32_t my3ACNumberOfThreatsStage4 = pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A );
					const uint32_t my4BCNumberOfThreatsStage4 = ( pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_AA ) +
																  pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_B )  +
																  pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_C ) );
...
bool StateEvaluation::IsHeadShot3AAPossible( const bool isMaxPlayer, Board::PositionXY& rBuildUpMove, const bool checkWinnerMoveCondition,
																									  const BoardScore* pCpuBoardScore,
																									  const BoardScore* pHumanBoardScore ) const
{
	bool retVal = false;
			...
			if( isHeadShootPossibleStage2 )
			{
				...
				if( isAdversary3A )
				{
					...
					if( commonMoveStage2 != POSITION_OUT_OF_BOARD )
					{
						...
						isAdversaryShutMyAttackStage2 = ( ( ( my4BCNumberOfThreatsStage2 < my4BCNumberOfThreatsStage1 ) && ( adversary4ANumberOfThreatsStage2 > 0 ) ) ||
													  	( pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_AA ) ) == 0 );
					}
				}
				...
				{
					// Here we know that we have 3AA & 4BC and adversary is not able to shut down my 4BC by its 3A (if having it).

					// Check if its possible to crate heat3A shoot using 3AA.
					Board::PositionXY buildUpMoveTmp;
					retVal = IsHeadShot3APossible( true, buildUpMoveTmp, false, pMyBoardStateCopy, pAdversaryBoardStateCopy );
...
bool StateEvaluation::IsHeadShotDragonPossible( const bool isMaxPlayer, Board::PositionXY& rBuildUpMove, const bool checkWinnerMoveCondition,
																									     const BoardScore* pCpuBoardScore,
																									     const BoardScore* pHumanBoardScore ) const
{
	bool retVal = false;
	...
	if( isHeadShootPossibleStage1 )
	{
		...
		for( auto iter = gaps3BC.begin(); iter != gaps3BC.end(); ++iter )
		{
			...
			const bool isHeadShot3A = IsHeadShot3APossible( true, rBuildUpMove, false, pMyBoardStateCopy, pAdversaryBoardStateCopy );
			...
			if( isHeadShot3A )
			{
				...
				const ThreatFinder::ThreatLocation threatLocation = pMyBoardStateCopy->GetThreatList( ThreatFinder::THREAT_3_CASE_A ).back();
...
bool StateEvaluation::IsHeadShotLizardPossible( const bool isMaxPlayer, Board::PositionXY& rBuildUpMove, const bool checkWinnerMoveCondition,
																										 const BoardScore* pCpuBoardScore,
																										 const BoardScore* pHumanBoardScore ) const
{
	bool retVal = false;
	...
	if( isHeadShootPossibleStage1 )
	{
		...
		ThreatFinder::KindOfThreats kindOfThreats[] = { ThreatFinder::THREAT_3_CASE_AA,
		...
		for( uint32_t i = 0; i < myNumberOf3BC-1 ; ++i )
		{
			for( uint32_t j = 0; j < maxGapSize ; ++j )
			{
				...
				for( uint32_t k = 0; k < maxGapSize ; ++k )
				{
					...
					// Head shot Dragon precedes 3A Head shot.
					const bool isDouble3APossible = IsDouble3AOneStrokePossible( true, rBuildUpMove, false, pMyBoardStateCopy, pAdversaryBoardStateCopy );
...
bool StateEvaluation::IsDouble3AOneStrokePossible( const bool isMaxPlayer, Board::PositionXY& rBuildUpMove, const bool checkWinnerMoveCondition,
																											const BoardScore* pCpuBoardScore,
																											const BoardScore* pHumanBoardScore ) const
{
	bool retVal = false;
	...
	const uint32_t my3A4ANumberOfThreatsInitStage0 = pMyBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A ) +
	...
	const bool isDouble3APossibleStage0 = ( isMy3A || isMy4ABC || isAdversary3A || isAdversary4ABC ) && checkWinnerMoveCondition;
	...
	const bool isDouble3APossibleStage1 = ( pMyBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_2_CASE_A ) > 1 );
	...
	if( isDouble3APossibleStage1 )
	{
		...
		for( auto it = duplicatedGaps2A.begin(); it != duplicatedGaps2A.end(); ++it )
		{
			...
			const uint32_t my3A4ANumberOfThreatsCurrentStage2 = pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A ) +
			...
			const bool isDouble3APossibleStage2 = ( my3A4ANumberOfThreatsCurrentStage2 >= ( my3A4ANumberOfThreatsInitStage0 + 2 ) );
			if( isDouble3APossibleStage2 )
			{
				...
				for( auto it = duplicatedGaps3A.begin(); it != duplicatedGaps3A.end(); ++it )
				{
					...
					const uint32_t my3A4ANumberOfThreatsCurrentStage3 = pMyBoardStateCopy2->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A ) +
...
bool StateEvaluation::IsDouble3AMitigationPossible( const bool isMaxPlayer, Board::PositionXY& rBuildDownMove, const bool checkWinnerMoveCondition,
																											   const BoardScore* pCpuBoardScore,
																											   const BoardScore* pHumanBoardScore ) const
{
	bool retVal = false;
	...
	const uint32_t my3A4ANumberOfThreatsInit = pMyBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A ) +
											   pMyBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_A ) ;

	// Don't consider mitigation if there is quick move to win.
	const bool is2x3AMitigationPossibleStage0 = ( isMy4ABC || isAdversary4ABC || isAdversary3A ) && checkWinnerMoveCondition;
	...
	const bool isMy2x3A			= ( pMyBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A ) > 1 );
	const bool isAdversary3BC   = ( pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_AA ) > 0 ||
	...
	if( is2x3AMitigationPossibleStage1 )
	{
		...
		for( auto iter = commonMovements.begin(); iter != commonMovements.end(); ++iter )
		{
			...
			if( adversary4ABCNumberOfThreatsInit < adversary4ABCNumberOfThreatsCurrent )
			{
				...
				const uint32_t my3A4ANumberOfThreatsCurrent = pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A ) +
															  pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_A );
				...
				if( !is2x3AMitigationPossibleStage2 )
				{
					...
					const bool isMy4A	= ( pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_A ) > 0 );
					const uint32_t my3A4ANumberOfThreatsCurrent = pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A ) +
																  pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_A );

					const bool is2x3AMitigationPossibleStage3 = ( my3A4ANumberOfThreatsCurrent == my3A4ANumberOfThreatsInit ) || isMy4A;
...
bool StateEvaluation::IsBlockAndAttack3APossible( const bool isMaxPlayer, Board::PositionXY& rBuildDownMove, const bool checkWinnerMoveCondition,
																											 const BoardScore* pCpuBoardScore,
																											 const BoardScore* pHumanBoardScore ) const
{
	bool retVal = false;
	...
	const bool isAdversary4ABC  = ( pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_A ) > 0 ||
									...
									pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_C ) > 0 );
	const bool isAdversary3A    = ( pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A ) > 0 );
	const bool isAdversary3BC   = ( pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_B ) > 0 ||
	...
	const uint32_t adversary3AThreatNumberInit  = pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A );
	const uint32_t adversary4BCThreatNumberInit = pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_AA ) +
												  pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_B ) +
										          pAdversaryBoardState->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_C );
	...
	if( is3AMitigationPossibleStage1 )
	{
		...
		for( auto iter = commonMovements.begin(); iter != commonMovements.end(); ++iter )
		{
			// Mitigation is possible if adversary is able shut down my 3A creating his 3A or 4BC threat at the same time.
			const Board::PositionXY newMove( m_pBoard->GetSize(), *iter );
			...
			const uint32_t my3AThreatNumber	        = pMyBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A );
			const uint32_t adversary3AThreatNumber  = pAdversaryBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_3_CASE_A );
			const uint32_t adversary4BCThreatNumber = pAdversaryBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_AA ) +
													  pAdversaryBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_B ) +
													  pAdversaryBoardStateCopy->GetNumberOfRecognizedThreat( ThreatFinder::THREAT_4_CASE_C );
			...
			const bool is3AMitigationPossibleStage1 = ( my3AThreatNumberInit > my3AThreatNumber ) &&
										 	 	 	  ( ( adversary3AThreatNumberInit < adversary3AThreatNumber ) || ( adversary4BCThreatNumberInit < adversary4BCThreatNumber ) );
...
int StateEvaluation::GetBonus( const bool isMaxPlayer, Board::PositionXY& rBuildDownMove, const bool checkWinnerMoveCondition,
																						  const BoardScore* pCpuBoardScore,
																						  const BoardScore* pHumanBoardScore ) const
{
	int retVal = 0;
	...
	const uint32_t common3A_2A = pMyBoardState->GetCommonFieldNumber( ThreatFinder::THREAT_3_CASE_A, ThreatFinder::THREAT_2_CASE_A );
	const uint32_t common2A_2A = pMyBoardState->GetCommonFieldNumber( ThreatFinder::THREAT_2_CASE_A, ThreatFinder::THREAT_2_CASE_A );
