...
class ClangFormat:
    ...
    def __init__(self, path: str, clang_ignore):
...
In clang_ignore file, all files/directories should be added in new line with */ before, eg.: 
*/src/ignored_dir/*
*/src/ignored_file.c
        """
    ...
    parser.add_argument("--clang_ignore", help="Config with files to ignore clang-format", default=None)
    parser.add_argument("--path", help="Path to repository with files to format.", required=True)

    args = vars(parser.parse_args())

    cf = ClangFormat(args["path"], args["clang_ignore"])
