...
const uint32_t Threat2CaseB::m_pov[] = {1, 2, 3, 4, 5 };

Threat2CaseB::Threat2CaseB() : ThreatFinder(PATTERN_LENGHT, m_pov, NUMELEM(m_pov), k_ATOM_NUMBER_2B)
{
    // nothing to do.
}
...
void Threat2CaseB::getThreatUpDetails(const IBoard::PositionXY initialPosition, const Trend trend,
                                      ThreatUpDetails & rThreatUpDetails) const
{
    ...
    else if(m_threatDownDetails.m_foundFlags.m_isSymmetricFound)
    {
        ...
        IBoard::PositionXY enemyTmp = rThreatUpDetails.m_myPawns[1];
